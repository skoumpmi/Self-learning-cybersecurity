...
class Training:
    ...
    def __init__( self, x_train, y_train, x_test, y_test, data, protocol, model_type, cv, db, attack):
    ...
    def classify(self):
        ...
        if (self.classifier_type == 'SDAE'):
                    ...
                    if self.protocol == 'MQTT':
                            ...
                            mqtt = self.db['mqtt_attacks']
                            if "mqtt_attacks" in self.db.list_collection_names():
                        ...
                        #num_classes = 4
                            ...
                            #db = myclient["labelled_netflows_db"]
                            modbus = self.db['modbus_attacks']
                            if "modbus_attacks" in self.db.list_collection_names():
                    ...
                    sdae = Sda(n_layers=2, num_epoch=1, en_act_func=['sigmoid', 'sigmoid', 'sigmoid', 'sigmoid'],
        ...
        if self.protocol == 'MQTT':
            ...
            if "mqtt_attacks" in db.list_collection_names():
                ...
                target_names = list(list(list(mqtt.find())[0].items())[1][1])
                
            else:
                 
                attackLabelsToInt = {"Label": {"Normal": 0, "Connection Overflow": 1, "Large Payload": 2, "Unauthorized Subscribe": 3}}
                if self.attack in list(list(attackLabelsToInt.values())[0].keys()):
                    target_names = ['Normal', 'Unauthorized Subscribe', 'Large Payload', 'Connection Overflow']
                else:
                    target_names = ['Normal', 'Unauthorized Subscribe', 'Large Payload', 'Connection Overflow', self.attack]
            ...
            if "modbus_attacks" in db.list_collection_names():
                ...
                target_names = list(list(list(modbus.find())[0].items())[1][1])
                ...
                if self.attack in list(list(attackLabelsToInt.values())[0].keys()):
                    ...
                    target_names = ['Normal', 'UID brute force', 'Enumeration Function', 'Fuzzing Read Holding Registers', self.attack]
