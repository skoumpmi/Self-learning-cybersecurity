...
from SDA_retrain import Sda_retrain
...
myclient = pymongo.MongoClient("mongodb://160.40.29.209:27017/")
#myclient = pymongo.MongoClient("mongodb://localhost:27017/")
#myclient = pymongo.MongoClient("mongodb://localhost:5002/")
#netflow_id = request_body['value']

    
db = myclient["labelled_netflows_db"]
#modbus_attacks = db["modbus_attacks"]
mqtt = db['mqtt_attacks']
modbus= db['modbus_attacks']
bacnet = db['bacnet_attacks']
ntp = db['ntp_attacks']
radius= db['radius_attacks']

class Training_ML:

    def __init__( self, x_train, y_train, x_test, y_test, data, protocol, model_type, cv, db):
    ...
    def classify(self, db):
        ...
        if(self.classifier_type == 'feature_extractor'):
            ...
            # save the classifier
            with open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + 'SDAE_Random_Forest_model.pkl', 'wb') as fid:
        ...
        if(self.classifier_type == 'SVC'):
            ...
            ##C = [0.1, 1, 10]
            C = [float(i)for i in config ['SVC']['params_C'].split(',')]
            ##kernel = ['linear', 'poly', 'rbf', 'sigmoid']
            kernel = config['SVC']['params_kernel'].split(',')
            ...
            # save the classifier
            with open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_" + self.classifier_type + '_model.pkl', 'wb') as fid:
                pickle.dump(SVC_model, fid)
            SVC_model = pickle.load(open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_" + self.classifier_type + '_model.pkl', 'rb'))
        ...
        if (self.classifier_type == 'KNN'):
                ...
                KNN_model = pickle.load(open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_" + self.classifier_type + '_model.pkl', 'rb'))
                prediction = KNN_model.predict(self.x_test)
                cm = confusion_matrix(self.y_test, prediction)
        
        
        if (self.classifier_type == 'DNN'):#TO DO_GET_DNN
                print('Training DNN model')
                dnn_model = self.get_DNN(self.x_train, self.y_train.to_numpy(), self.data, self.protocol)
                print(dnn_model.summary())
                # save model and architecture to single file
                dnn_model.save("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/" + self.protocol + "_DNN_model.h5")
        ...
        if self.protocol == 'MQTT':
            ...
            if "mqtt_attacks" in db.list_collection_names():
                mqtt = db['mqtt_attacks']
                target_names = list(list(mqtt.find()[0].items())[1][1].values())#[1:]
            else:
            ...
            if "modbus_attacks" in db.list_collection_names():
                modbus = db['modbus_attacks']
                target_names = list(list(modbus.find()[0].items())[1][1].values())#[1:]
            else:
            ...
            if "bacnet_attacks" in db.list_collection_names():
                bacnet = db['bacnet_attacks']
                target_names = list(list(bacnet.find()[0].items())[1][1].values())#[1:]
            else:
            ...
            if "ntp_attacks" in db.list_collection_names():
                ntp = db['ntp_attacks']
                target_names = list(list(ntp.find()[0].items())[1][1].values())#[1:]
            else:
            ...
            if "radius_attacks" in db.list_collection_names():
                radius = db['radius_attacks']
                target_names = list(list(radius.find()[0].items())[1][1].values())#[1:]
            else:
        ...
        cm.to_csv('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/reports/' + self.protocol + "_" + self.classifier_type + "_confusion_matrix.csv")
        ...
        figure.savefig('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/reports/' + self.protocol + "_" + self.classifier_type + '_confusion_matrix_heat')
    ...
    def test(self):
        if self.protocol == 'MQTT':
            print('MQTT network flow classification test')
            csv = pd.read_csv("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/testing/MQTT_test.csv")
            mqtt = db['mqtt_attacks']
            if "mqtt_attacks" in db.list_collection_names():
                    
                    attackLabelsToInt = {"Label": list(list(mqtt.find())[0].items())[1][1]}
                    target_names = list(list(mqtt.find()[0].items())[1][1].values())[1:]
            else:
                attackLabelsToInt = {
            ...
            csv = pd.read_csv("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/datasets/modbus_dataset.csv")
            modbus = db['modbus_attacks']
            if "modbus_attacks" in db.list_collection_names():
                    
                    attackLabelsToInt = {"Label": list(list(modbus.find())[0].items())[1][1]}
                    target_names = list(list(modbus.find()[0].items())[1][1].values())[1:]
            else:
                attackLabelsToInt = {
                        "Label": {"Normal": 0, 'UID brute force': 1, 'Enumeration Function': 2, 'Fuzzing Read Holding Registers': 3}}
                target_names = [ 'UID brute force', 'Enumeration Function', 'Fuzzing Read Holding Registers']
        elif self.protocol == 'BACNET':
            ...
            csv = pd.read_csv("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/testing/BACnet_test.csv")
            bacnet = db['bacnet_attacks']
            if "bacnet_attacks" in db.list_collection_names():
                    
                    attackLabelsToInt = {"Label": list(list(bacnet.find())[0].items())[1][1]}
                    target_names = list(list(bacnet.find()[0].items())[1][1].values())[1:]
            else:
                attackLabelsToInt = {
                        "Label": {"Normal": 0, "BACnet Fuzzing": 1, "Tampering": 2, "Flooding": 3}}
                target_names = ['BACnet Fuzzing', 'Tampering', 'Flooding']
            ...
            csv = pd.read_csv("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/testing/NTP_test.csv")
            ntp = db['ntp_attacks']
            if "ntp_attacks" in db.list_collection_names():
                    
                    attackLabelsToInt = {"Label": list(list(ntp.find())[0].items())[1][1]}
                    target_names = list(list(ntp.find()[0].items())[1][1].values())[1:]
            else:
                attackLabelsToInt = {
                        "Label": {"Normal": 0, "KissOfDeath": 1, "TimeSkimming": 2}}
                target_names = ['KissOfDeath', 'TimeSkimming']
            ...
            csv = pd.read_csv("home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/testing/Radius_test.csv")
            radius = db['radius_attacks']
            if "radius_attacks" in db.list_collection_names():
                    
                    attackLabelsToInt = {"Label": list(list(radius.find())[0].items())[1][1]}
                    target_names = list(list(radius.find()[0].items())[1][1].values())[1:]
            else:
                attackLabelsToInt = {
                        "Label": {"Normal": 0, "Brute Force": 1}}
                target_names = ['Brute Force']
        ...
        # load the scaler
        scaler = load(open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/scalers/' + self.protocol + "_" + self.classifier_type + '_scaler.pkl', 'rb'))
        # transform the test dataset
        X_scaled = scaler.transform(X)
        if self.classifier_type == 'SDAE':
            model = load_model("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/" + self.protocol + "_" + self.classifier_type + "_model.h5")
            prediction = model.predict_classes(X_scaled)
        else:
            model = pickle.load(open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_" + self.classifier_type + '_model.pkl', 'rb'))
    ...
    def classify(self,db):
        ...
        if (self.classifier_type == 'SDAE'):
                    ...
                    if self.protocol == 'MQTT':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                            ...
                            if "mqtt_attacks" in db.list_collection_names():
                                mqtt = db['mqtt_attacks']
                                num_classes = len(list(list(mqtt.find()[0].items())[1][1].values()))
                                print('number of classes is',num_classes)
                            else:
                                num_classes = 4
                                print('number of classes is is',num_classes)
                    
                    elif self.protocol == 'MODBUS':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                        #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                            ##db = myclient["labelled_netflows_db"]
                            #modbus_attacks = db["modbus_attacks"]
                            ...
                            if "modbus_attacks" in db.list_collection_names():
                                modbus = db['modbus_attacks']
                                num_classes = len(list(list(modbus.find()[0].items())[1][1].values()))
                                print('number of classes is',num_classes)
                            else:
                                num_classes = 4
                                print('number of classes is is',num_classes)
                    elif self.protocol == 'BACNET':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                        #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                            ##db = myclient["labelled_netflows_db"]
                            #modbus_attacks = db["modbus_attacks"]
                            ...
                            if "bacnet_attacks" in db.list_collection_names():
                                bacnet = db['bacnet_attacks']
                                num_classes = len(list(list(bacnet.find()[0].items())[1][1].values()))
                                print('number of classes is',num_classes)
                            else:
                                num_classes = 4
                                print('number of classes is is',num_classes)
                            ...
                            if "ntp_attacks" in db.list_collection_names():
                                ntp = db['ntp_attacks']
                                num_classes = len(list(list(ntp.find()[0].items())[1][1].values()))
                                print('number of classes is',num_classes)
                            else:
                                num_classes = 3
                                print('number of classes is is',num_classes)
                        ...
                        #num_classes = 3
                            ...
                            if "radius_attacks" in db.list_collection_names():
                                radius = db['radius_attacks']
                                num_classes = len(list(list(radius.find()[0].items())[1][1].values()))
                                print('number of classes is',num_classes)
                            else:
                                num_classes = 2
                                print('number of classes is is',num_classes)
                    ...
                    sdae = Sda_retrain(n_layers=int(config['SDAE']['n_layers']), 
                    ...
                    pretrained_sdae_model = tf.keras.models.load_model('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_feature_extractor.h5")
                    ...
                    # save model and architecture to single file
                    sdae_model.save('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/models/' + self.protocol + "_" + self.classifier_type + '_model.h5')
    ...
    def test(self):
        if self.protocol == 'MQTT':
            ...
            if self.protocol == 'MQTT':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                ...
                mqtt = db['mqtt_attacks']
                if "mqtt_attacks" in db.list_collection_names():
                    target_names = (list(list(mqtt.find()[0].items())[1][1].values()))
                    attackLabelsToInt = {"Label": list(list(mqtt.find())[0].items())[1][1]}
                    #print('number of classes is',num_classes)
                else:
                    attackLabelsToInt = {
            ...
            modbus = db['modbus_attacks']
            #mqtt = db['mqtt_attacks']
            if self.protocol == 'MODBUS':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                db = myclient["labelled_netflows_db"]
                #modbus_attacks = db["modbus_attacks"]
                modbus = db['modbus_attacks']
                if "modbus_attacks" in db.list_collection_names():
                    target_names = (list(list(modbus.find()[0].items())[1][1].values()))
                    attackLabelsToInt = {"Label": list(list(modbus.find())[0].items())[1][1]}
                    #print('number of classes is',num_classes)
                else:
                    attackLabelsToInt = {
                    ...
                    #num_classes = 4
                    #print('number of classes is is',num_classes)
                #'Normal', 'UID brute force', 'Enumeration Function', 'Fuzzing Read Holding Registers'
                print('MODBUS network flow classification test')
                csv = pd.read_csv("/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self-learning_retrain/datasets/modbus_dataset.csv")
            ...
            bacnet = db['bacnet_attacks']
            #mqtt = db['mqtt_attacks']
            if self.protocol == 'BACNET':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                ##db = myclient["labelled_netflows_db"]
                #modbus_attacks = db["modbus_attacks"]
                bacnet = db['bacnet_attacks']
                if "bacnet_attacks" in db.list_collection_names():
                    target_names = (list(list(bacnet.find()[0].items())[1][1].values()))
                    attackLabelsToInt = {"Label": list(list(bacnet.find())[0].items())[1][1]}
                    #print('number of classes is',num_classes)
                else:
                    attackLabelsToInt = {
            ...
            ntp  = db['ntp_attacks']
            #mqtt = db['mqtt_attacks']
            if self.protocol == 'NTP':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                db = myclient["labelled_netflows_db"]
                #modbus_attacks = db["modbus_attacks"]
                ntp = db['ntp_attacks']
                if "ntp_attacks" in db.list_collection_names():
                    target_names = (list(list(ntp.find()[0].items())[1][1].values()))
                    attackLabelsToInt = {"Label": list(list(ntp.find())[0].items())[1][1]}
                    #print('number of classes is',num_classes)
                else:
                    attackLabelsToInt = {
            ...
            radius  = db['radius_attacks']
            #mqtt = db['mqtt_attacks']
            if self.protocol == 'RADIUS':# or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                #if self.protocol == 'MQTT' or self.protocol == 'BACNET' or self.protocol == 'MODBUS':
                db = myclient["labelled_netflows_db"]
                #modbus_attacks = db["modbus_attacks"]
                radius = db['radius_attacks']
                if "radius_attacks" in db.list_collection_names():
                    target_names = (list(list(radius.find()[0].items())[1][1].values()))
                    attackLabelsToInt = {"Label": list(list(radius.find())[0].items())[1][1]}
                    #print('number of classes is',num_classes)
                else:
                    attackLabelsToInt = {
