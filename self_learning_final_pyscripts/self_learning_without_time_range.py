...
def internal_server_error(e):
    ...
    ##return render_template('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/templates/500.html'), 500
...
def TrainingModels():
    ...
    for item in request_body['data']:
        ...
        print('====================')
        seq = item['id'].split('-')
        x = pd.concat([x, pd.DataFrame(list(modbus.find({"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]})))]).reset_index(drop=True)
        ...
        if x.empty:
            ...
            modbus.update_many({"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]},
                               {
                                   "$set": {"Label": item['label']},
                                   # $currentDate: { lastModified: true }
                               })
         
            x1 = pd.DataFrame(list(modbus.find({"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]}))).reset_index(drop=True)
            if x1.empty:
                pass
            else:
        ...
        if y.empty:
            ...
            mqtt.update_many(
                {"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]},
                    {
                        "$set": {"Label": item['label']},
                        # $currentDate: { lastModified: true }
                    }
                )
            #y1 = pd.DataFrame(list(mqtt.find({"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]}))).reset_index(drop=True)
            y1 = pd.DataFrame(list(mqtt.find({"$or": [{"Src IP":seq[0], "Dst IP":seq[1]}, {"Dst IP":seq[0],"Src IP":seq[1]}]}))).reset_index(drop=True)
            if y1.empty:
                pass
            else:
    ...
    for protocol in protocols:
        ...
        if protocol in protocols:
            ...
            #return render_template('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/templates/400_protocol.html'), 400
        ...
        if (modelTypes in all_models) or (set(modelTypes).issubset(set(all_models))):
            ...
            #return render_template('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/templates/400_model.html'), 400
...
def trainFromScratch(protocol, models, cv, num_classes, attackLabelsToInt, target_names):
    ...
    if best_model == 'SDAE':
        ...
        with open(os.path.join(os.getcwd(), '..//best_models/{}/'.format(protocol) + protocol + "_" + best_model + '_model.pkl'), 'wb')as fid:    
...
def parallelTrain(x_train, y_train, x_test, y_test,csv, protocol, model_type, cv, num_classes, attackLabelsToInt, target_names):
    ...
    if model_type == 'SDAE':
        ...
        model.save(os.path.join(os.getcwd(),'../models/' + protocol + "_" + model_type + '_model.h5'))
        ...
        with open(os.path.join(os.getcwd(),'../models/' + protocol + "_" + model_type + '_model.pkl', 'wb')) as fid:
            pickle.dump(model, fid) 
    ...
    for tup in results:
        ...
        if tup[2] == 'iti-572':
                ...
                if tup[0] == 'SDAE':
                    ...
                    #sftp.get('{}_{}_model.pkl'.format(protocol, tup[0]),'/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/models/{}_{}_model.pkl'.format(protocol, tup[0])) 
                    sftp.get('{}_{}_model.pkl'.format(protocol, tup[0]),(os.path.join(os.getcwd(),'../models/{}_{}_model.pkl'.format(protocol, tup[0]))))
