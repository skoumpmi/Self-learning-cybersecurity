...
class Data:
    ...
    def scaling(self, model_type):
        #if not os.path.exists('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/scalers/' + str(self.protocol) + "_" + str(model_type) +'_scaler.pkl'):
        if not os.path.exists(os.path.join(os.getcwd(),'../scalers/' + str(self.protocol) + "_" + str(model_type) +'_scaler.pkl')):
            print('scaler for ', str(self.protocol), 'and', str(model_type), 'does not exist')
            scaler = StandardScaler()
            X = scaler.fit(self.data).transform(self.data)
            #with open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/scalers/' + str(self.protocol) + "_" + str(model_type) + '_scaler.pkl', 'wb') as fid:
            with open(os.path.join(os.getcwd(),'../scalers/' + str(self.protocol) + "_" + str(model_type) + '_scaler.pkl'), 'wb') as fid:
                pickle.dump(scaler, fid)

        print('scaler for ', str(self.protocol),' exists')
        #load the scaler
        #scaler = load(open('/home/smarthome/SPEAR/varlab_dev/SPEAR/development/BDAC/Self_learning_retrain/scalers/' + str(self.protocol) + "_" + str(model_type) +'_scaler.pkl', 'rb'))
        scaler = load(open(os.path.join(os.getcwd(),'../scalers/' + str(self.protocol) + "_" + str(model_type) +'_scaler.pkl')), 'rb')
        X = scaler.fit(self.data).transform(self.data)
        return X
